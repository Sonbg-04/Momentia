    ...
    <div className="px-2 pb-3 border-b">
      <div className="space-y-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${selectedOption.color}`}
                />
        ...
        <div className="flex justify-between items-center text-xs text-muted-foreground bg-muted/30 rounded-md px-2 py-1.5">
          <span>
            Hoạt động: {filterOptions.filter((f) => f.count > 0).length}
          </span>
          <span>
            Tổng: {filterOptions.reduce((sum, option) => sum + option.count, 0)}
          </span>
        </div>
      </div>
    </div>
