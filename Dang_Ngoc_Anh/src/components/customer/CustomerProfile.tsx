...
// import { CustomerWallet } from "./CustomerWallet"; - removed wallet import
...
const FavoritePhotographersView: React.FC<{
  ...
  const openChat = (p: Photographer) =>
    alert(`Mở chat với ${p.name}`);
  const removeFavorite = (p: Photographer) =>
    alert(`Bỏ yêu thích: ${p.name}`);
  const viewProfile = (p: Photographer) =>
    alert(`Xem hồ sơ của ${p.name}`);
  const bookNow = (p: Photographer) =>
    alert(`Đặt lịch với ${p.name}`);
...
const ChangePasswordView: React.FC<{ onBack?: () => void }> = ({
  onBack,
}) => {
  ...
  const submit = () => {
    ...
    if (strength < 3)
      return alert("Mật khẩu mới chưa đủ mạnh (tối thiểu 3/5)");
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Gợi ý: dùng chữ hoa, số và ký tự đặc biệt để
                mạnh hơn.
              </p>
            </div>
            <div className="space-y-2">
        ...
        <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
          ...
          <CardContent className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Gửi thông báo khi tài khoản đăng nhập trên thiết
                bị lạ.
              </p>
            </div>
          </CardContent>
...
export function CustomerProfile({
  ...
  const openSupportChat = () =>
    alert("Mở chat hỗ trợ khách hàng");

  // Sub-routes
  if (currentView === "favorites") {
    return (
      <FavoritePhotographersView
