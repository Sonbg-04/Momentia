  ...
  const allPhotographers = [
    {
      ...
      isVerified: true,
      level: 'Expert',
      coverImage: 'https://images.unsplash.com/photo-1519741497674-611481863552?w=400&h=200&fit=crop&crop=center',
      completedBookings: 250,
      responseTime: '< 1 giờ',
      joinedDate: '2023-01-15',
      description: 'Chuyên gia chụp ảnh cưới và gia đình với phong cách tự nhiên lãng mạn'
    },
    {
      id: 2,
      name: 'Thu Hương',
      ...
      isVerified: true,
      level: 'Professional',
      coverImage: 'https://images.unsplash.com/photo-1606216794074-735e91aa2c92?w=400&h=200&fit=crop&crop=center',
      completedBookings: 180,
      responseTime: '< 2 giờ',
      joinedDate: '2023-03-20',
      description: 'Nhiếp ảnh gia chân dung và thời trang với phong cách nghệ thuật'
    },
    {
      id: 3,
      name: 'Thu Hà',
      ...
      isVerified: true,
      level: 'Master',
      coverImage: 'https://images.unsplash.com/photo-1511285560929-80b456fea0bc?w=400&h=200&fit=crop&crop=center',
      completedBookings: 340,
      responseTime: '< 30 phút',
      joinedDate: '2022-08-10',
      description: 'Chuyên gia chụp ảnh sự kiện doanh nghiệp với 8+ năm kinh nghiệm'
    },
    {
      id: 4,
      name: 'Minh Tuấn',
      ...
      isVerified: false,
      level: 'Professional',
      coverImage: 'https://images.unsplash.com/photo-1542038784456-1ea8e935640e?w=400&h=200&fit=crop&crop=center',
    ...
    },
    {
      ...
      isVerified: true,
      level: 'Professional',
      coverImage: 'https://images.unsplash.com/photo-1469371670807-013ccf25f16a?w=400&h=200&fit=crop&crop=center',
    ...
    },
    {
      ...
      isVerified: true,
      level: 'Master',
      coverImage: 'https://images.unsplash.com/photo-1465495976277-4387d4b0e4a6?w=400&h=200&fit=crop&crop=center',
  ...
  const filteredPhotographers = useMemo(() => {
    let filtered = allPhotographers.filter(photographer => {
      ...
      if (selectedCategory !== 'all') {
        ...
        const relevantSpecialties = categoryMap[selectedCategory as keyof typeof categoryMap] || [];
        if (!photographer.specialties.some(spec => relevantSpecialties.includes(spec))) {
          return false;
        }
      }
      ...
      if (searchQuery) {
        ...
        if (!photographer.name.toLowerCase().includes(query) &&
            !photographer.location.toLowerCase().includes(query) &&
            !photographer.specialties.some(spec => spec.toLowerCase().includes(query)) &&
      ...
      if (priceRange) {
        ...
        if (range && (photographer.priceValue < range.min || photographer.priceValue > range.max)) {
          return false;
        }
      }
    ...
    return filtered;
  }, [selectedCategory, searchQuery, priceRange, locationFilter, ratingFilter, sortBy]);
    ...
    <div className="p-4 sm:p-6 space-y-8">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            <div className="flex-1 text-center lg:text-left">
              ...
              <p className="text-lg text-muted-foreground mb-6 max-w-lg mx-auto lg:mx-0">
                Kết nối với những nhiếp ảnh gia tài năng nhất và tạo ra những khoảnh khắc đáng nhớ
              </p>
            ...
            <div className="flex-shrink-0">
              <div className="relative">
                ...
                <div className="absolute -bottom-2 -left-4 w-12 h-12 bg-pink-500 rounded-full flex items-center justify-center shadow-lg animate-pulse-glow" style={{animationDelay: '1s'}}>
      ...
      <div ref={searchSectionRef} className="space-y-6">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Tìm nhiếp ảnh gia chuyên nghiệp cho từng loại hình chụp ảnh</p>
        </div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="border-primary/20">
                    ...
                    {priceRanges.find(r => r.value === priceRange)?.label || 'Giá'}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  {locations.map((location) => (
                    <DropdownMenuItem 
                      key={location} 
                      onClick={() => setLocationFilter(location === 'Tất cả' ? null : location)}
                    >
                      {location}
                    </DropdownMenuItem>
                  ))}
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="border-primary/20">
                    ...
                    {ratingFilters.find(r => r.value === ratingFilter)?.label || 'Đánh giá'}
                    <ChevronDown className="w-4 h-4 ml-2" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent>
          ...
          <div className="mt-4 pt-4 border-t border-primary/10">
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                Tìm thấy <span className="font-semibold text-foreground">{filteredPhotographers.length}</span> nhiếp ảnh gia
              ...
              {(priceRange || locationFilter || ratingFilter) && (
                <div className="flex items-center gap-2">
                  ...
                  {ratingFilter && (
                    <Badge variant="secondary" className="text-xs">
                      {ratingFilters.find(r => r.value === ratingFilter)?.label}
                      <button 
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Những tài năng được đánh giá cao nhất trên nền tảng</p>
          </div>
        ...
        {filteredPhotographers.length === 0 ? (
          ...
          <>
            <div className={`gap-6 ${viewMode === 'grid' ? 'grid sm:grid-cols-2 lg:grid-cols-3' : 'space-y-6'}`}>
                  ...
                  >
                    <div className="relative overflow-hidden">
                      ...
                      <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                        <Button size="sm" variant="secondary" className="w-9 h-9 p-0 backdrop-blur-sm bg-white/20 hover:bg-white/30 border-white/20">
                        ...
                        <div className="flex items-center gap-1">
                          <div className="flex">
                            {[...Array(5)].map((_, i) => (
                              <Star 
                                key={i} 
                                className={`w-4 h-4 ${i < Math.floor(photographer.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} 
                              />
                            ))}
                          </div>
                          <span className="font-semibold text-sm ml-1">{photographer.rating}</span>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-6 mb-3">
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-muted-foreground">({photographer.reviews} đánh giá)</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <Camera className="w-4 h-4 text-muted-foreground" />
                              <span className="text-muted-foreground">{photographer.completedBookings} buổi chụp</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <Clock className="w-4 h-4 text-muted-foreground" />
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <span className="font-semibold text-xl text-primary">{photographer.price}</span>
                            </div>
                            <div className="flex gap-2">
          ...
          <CardContent className="p-8 text-center">
            ...
            <div className="relative">
              ...
              <p className="text-primary-foreground/80 mb-6 max-w-md mx-auto">
                Hàng nghìn nhiếp ảnh gia tài năng đang chờ để ghi lại những khoảnh khắc đẹp nhất của bạn
              </p>
              <Button 
                size="lg" 
                variant="secondary" 
