...
import { PhotographerAuth } from "./Auth/PhotographerAuth";
...
export function PhotographerApp({
    ...
    isAuthenticated,
    onLogin,
}: PhotographerAppProps) {
    const [currentView, setCurrentView] = useState<PhotographerView>("home");

    if (!isAuthenticated) {
        return <PhotographerAuth onLogin={onLogin} onBack={onBack} />;
    }
        ...
        switch (currentView) {
            case "home":
                return (
                    <PhotographerHome
                        onNavigate={(view) =>
                            setCurrentView(view as PhotographerView)
                        }
                    />
                );
            case "bookings":
            ...
            case "messages":
                return (
                    <PhotographerChat onBack={() => setCurrentView("home")} />
                );
            case "profile":
                return (
                    <PhotographerProfile
